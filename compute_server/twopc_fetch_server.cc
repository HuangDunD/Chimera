...
void ComputeServer::Write_2pc_data(node_id_t node_id, table_id_t table_id, Rid rid, char* data){
    assert(SYSTEM_MODE == 9);
...
void ComputeServer::Abort_2pc(std::unordered_map<node_id_t, std::vector<std::pair<table_id_t, Rid>>> node_data_map, uint64_t txn_id){
    assert(SYSTEM_MODE == 9);
    std::vector<brpc::CallId> cids;
...
int ComputeServer::Commit_2pc(std::unordered_map<node_id_t, std::vector<std::pair<std::pair<table_id_t, Rid>, char*>>> node_data_map, uint64_t txn_id){
    assert(SYSTEM_MODE == 9);
    std::vector<brpc::CallId> cids;
    int c = 0;
    for(auto node_data: node_data_map){
